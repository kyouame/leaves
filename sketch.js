...
function preload() {
  ...
  windSound = loadSound('wind-and-leaves.mp3');
}
...
function draw() {
  ...
  // 🌬️ 淡入淡出音量控制
  let targetVolume = isDragging ? 0.8 : 0;
  currentVolume = lerp(currentVolume, targetVolume, 0.05);
  windSound.setVolume(currentVolume);

  // 🌬️ 若淡出到接近無聲，停止播放
  if (!isDragging && currentVolume < 0.01 && windSound.isPlaying()) {
    windSound.stop();
  }
